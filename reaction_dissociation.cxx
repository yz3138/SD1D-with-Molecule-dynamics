...
//    Sn_1=0.0;
    CELL_AVERAGE(i,                          // Index variable
      ...
      BoutReal R = Ne * Nm * (hydrogen.Diss_1(Te*Tnorm)+hydrogen.Diss_2(Te*Tnorm)) * (Nnorm / Omega_ci);  // dissociation ionization (DI)
...
//      Sn_1[i] += weight * R1;
  ...
  SourceMap densitySources() override {
    return {{"h", 2*Sn},   ///????  why is not '2*Sn' for h, and 'Sn' for h2????
            {"h2", -Sn}
            }; // Redistribute neutrals
  }
