...
BoutReal UpdatedRadiatedPower::Ion_H2(BoutReal T) {
  ...
  double Ion_H2_RateCoefficient,rate_c,TT;
  TT = T;
  double Ion_H2_coeffs[9]= {-3.56864029E1, 1.73346898E1, -7.76746936, 2.21157940,-4.16984017E-1, 5.08828982E-2, -4.83273751E-3,1.61286312E-4, -2.89339190E-6};
...
/// !!!!! the error is typing wrong data'-3.832737518E-3' here, which should be '-4.83273751E-3'.
...
BoutReal UpdatedRadiatedPower::Ion_charged_H2(BoutReal T) {
  ...
  double Ion_charged_H2_RateCoefficient,rate_i,TT;
  TT = T;
...
//  9.H2+ ionized by collision with electrons
//    e + H2+ -> 2e + H + H+

BoutReal UpdatedRadiatedPower::Ion_charged_H2_1(BoutReal T) {
  if (T < 0.2) {
    T = 0.2;
  }
  double Ion_charged_H2_RateCoefficient_1,rate_i_1,TT;
  TT = T;

  double Ion_charged_H2_coeffs_1[9]= {-1.781416067709E1, 2.277799785711, -1.266868411626, 4.296170447419E-1,-9.609908013189E-2, 1.387958040699E-2, -1.231349039470E-3,6.042383126281E-5, -1.247521040900E-6};
  Ion_charged_H2_RateCoefficient_1=0.0;
  for (int i=0;i<=8;i++){
   Ion_charged_H2_RateCoefficient_1 += Ion_charged_H2_coeffs_1[i]*pow(log(TT),i);
}
rate_i_1 = exp(Ion_charged_H2_RateCoefficient_1)*1.0e-6;
return rate_i_1;
}
...
//  10.H2+ ionized by collision with electrons
//    e + H2 -> e + H + H

BoutReal UpdatedRadiatedPower::Diss_1(BoutReal T) {
  if (T < 1.26) {
    T = 1.26;
  }
  double Diss_RateCoefficient,rate_Diss_1,TT;
  TT = T;

  double Diss_coeffs[9]= {-2.8580728365E1,1.038543976E1,-5.3838250265,1.9506364944,-5.3936663924E-1,1.006916814453E-1,-1.16075857397E-2,7.411623859122E-4,-2.001369618807E-5};
  Diss_RateCoefficient=0.0;
  for (int i=0;i<=8;i++){
   Diss_RateCoefficient += Diss_coeffs[i]*pow(log(TT),i);
}
rate_Diss_1 = exp(Diss_RateCoefficient)*1.0e-6;
return rate_Diss_1;
}

//  11.H2+ ionized by collision with electrons
//    e + H2 -> e + H + H

BoutReal UpdatedRadiatedPower::Diss_2(BoutReal T) {
  if (T < 2.5) {
    T = 2.5;
  }
  double Diss_RateCoefficient_2,rate_Diss_2,TT;
  TT = T;

  double Diss_coeffs_2[9]= {-3.454175591367E1,1.41265591128E1,-6.004466156761,1.589476697488,-2.775796909649E-1,3.152736888124E-2,-2.229578042005E-3,8.890114963166E-5,-1.523912962346E-6};
  Diss_RateCoefficient_2=0.0;
  for (int i=0;i<=8;i++){
   Diss_RateCoefficient_2 += Diss_coeffs_2[i]*pow(log(TT),i);
}
rate_Diss_2 = exp(Diss_RateCoefficient_2)*1.0e-6;
return rate_Diss_2;
}


BoutReal UpdatedRadiatedPower::zhwang(BoutReal T) {
  double nu_diss;  //<sigma*v> [m3/s]
  double TT;

  TT=T;

  if (TT<1.0) TT=1.0;
  nu_diss=3.e3*TT*TT/(3.+0.01*TT*TT);
