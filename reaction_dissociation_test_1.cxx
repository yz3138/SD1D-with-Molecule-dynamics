...
class ReactionHydrogenzhwang : public Reaction {
public:
  ReactionHydrogenzhwang(Options *options) {
    AUTO_TRACE();
...
//    Sn_1=0.0;
    CELL_AVERAGE(i,                          // Index variable
      ...
      BoutReal nu_diss = 3.e3*Te*Te*Tnorm*Tnorm/(3.+0.01*Te*Te*Tnorm*Tnorm);
      BoutReal R = Ne * Nm * nu_diss * (Nnorm / Omega_ci);  // dissociation ionization (DI)
...
//      Sn_1[i] += weight * R1;
  ...
  std::string str() const { return "Hydrogen Diss from Z.H.Wang"; }
...
namespace {
RegisterInFactory<Reaction, ReactionHydrogenzhwang> register_rc("hydrogen_diss_ion_zhwang");
}
