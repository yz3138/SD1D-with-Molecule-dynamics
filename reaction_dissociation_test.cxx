...
class ReactionHydrogenDI : public Reaction {
public:
  ReactionHydrogenDI(Options *options) {
    AUTO_TRACE();
...
//    Sn_1=0.0;
    CELL_AVERAGE(i,                          // Index variable
      ...
      BoutReal R = Ne * Nm * hydrogen.Ion_charged_H2(Te*Tnorm) * (Nnorm / Omega_ci);  // dissociation ionization (DI)
...
//      Sn_1[i] += weight * R1;
  ...
  std::string str() const { return "Hydrogen DI"; }
...
namespace {
RegisterInFactory<Reaction, ReactionHydrogenDI> register_rc("hydrogen_diss_ion");
}
