...
class ReactionLossRate_H2 : public Reaction {
public:
  ReactionLossRate_H2(Options *options) {
    AUTO_TRACE();
  ...
  void updateSpecies(const SpeciesMap &species, BoutReal UNUSED(Tnorm), BoutReal UNUSED(Nnorm),
    ...
    // Get the species
    auto &molecules = *species.at(name);
    
    // Extract required variables
    Field3D N{molecules.N}, NV{molecules.NV}, P{molecules.P};
  ...
  std::string str() const { return "Neutral loss for H2"; }
...
namespace {
RegisterInFactory<Reaction, ReactionLossRate_H2> register_nl("fixed_loss_rate_h2");
}
